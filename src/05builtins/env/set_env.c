...
/*   Updated: 2024/04/09 21:52:55 by lannur-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_env(t_core_struct *core, char *key_value_pair)
{
	...
	value_to_set = ft_strtrim(get_value(key_value_pair), " ");
	while (core->env_list)
	{
		...
		else
		{
				...
				&& (ft_strcmp((core->env_list)->name, key_to_set) == 0))
			{
				if (*value_to_set != '\0')
				{
					replace_val(&((core->env_list)->value), value_to_set);
				}
				else
				{
					replace_val(&((core->env_list)->value), "");
				}
				modified_flag = 1;
			}
	...
	if (value_to_set != NULL && modified_flag == 0)
	{
		core->env_list = head;
