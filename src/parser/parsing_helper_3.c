...
/*   Updated: 2024/03/20 13:03:18 by trysinsk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    (*cmd) = create_redircmd((*cmd), (tok)->value, O_WRONLY | O_CREAT | O_TRUNC, 1);
	*core->token_head = advance_token(&tok);
	if (((*core->token_head)->type != T_REDIRECT_IN) && \
		((*core->token_head)->type != T_REDIRECT_OUT) && \
		((*core->token_head)->type != T_APPEND_OUT))
...
{
    (*cmd) = create_redircmd((*cmd), (tok)->value, O_WRONLY | O_CREAT, 1);
	*core->token_head = advance_token(&tok);
	if (((*core->token_head)->type != T_REDIRECT_IN) && \
		((*core->token_head)->type != T_REDIRECT_OUT) && \
		((*core->token_head)->type != T_APPEND_OUT))
